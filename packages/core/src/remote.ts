...
import { InvokeError, invokeRegistryFunction } from "./invoke";
...
export const defineRemoteFunctions = <
...
export const renderRemoteModule = (
  ...
  [
    ...
    'import { defineRemoteFunctions } from "voke";',
    "",
    `export const ${generatedIdentifier(name)} = defineRemoteFunctions(${JSON.stringify(name)}, ${JSON.stringify(artifact, null, 2)} as const);`,
    "",
  ].join("\n");
